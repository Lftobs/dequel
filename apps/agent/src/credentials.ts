...
type StoredCredential = { serverId: string; agentId: string; credential: string; wireguard: WireGuardPeerConfig | null };
...
export const registerAgent = async (capabilities: AgentCapabilities): Promise<StoredCredential> => {
  ...
  const response = await fetch(`${requireControlPlaneUrl()}/api/agents/register`, {
    ...
    body: JSON.stringify({ token: config.registrationToken, agentVersion: config.agentVersion, capabilities, publicHost: config.publicHost }),
  });
  if (!response.ok) {
