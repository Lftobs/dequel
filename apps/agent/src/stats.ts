...
export const collectContainerStats = async (): Promise<AgentContainerStat[]> => {
	...
	try {
		...
		for (const id of ids) {
			...
			const memStr =
				String(parsed.MemUsage ?? "0B")
					.split("/")[0]
					?.trim() ?? "0B";
			const projectId = await run("docker", [
				"inspect",
				"--format",
				'{{index .Config.Labels "com.dequel.project"}}',
				id,
			]).catch(() => "");
			const replica = await run("docker", [
				"inspect",
				"--format",
				'{{index .Config.Labels "com.dequel.replica"}}',
				id,
			]).catch(() => "");
			stats.push({
				containerName: name,
