...
mock.module(fileUrl("../../db/repo"), () => ({
	...
	createAgentJob: mock(async (input: any) => {
		mockScaleJobs.push(input);
		return "job-1";
	}),
...
describe("evaluate", () => {
	...
	it("scales up through a scale job on an online agent server", async () => {
		...
		mockRedisData = {
			...
			"dequel:agent-stats:server-1": JSON.stringify({
				...
				containers: [{ containerName: "deploy-dep-1", cpuPercent: 90, memoryMb: 200 }],
			}),
		};
		mockScaleJobs = [];
	...
	it("counts agent replicas from cached stats", async () => {
		mockRedisData = {
			"dequel:agent-stats:server-1": JSON.stringify({
				...
				containers: [
					...
					{
						containerName: "deploy-dep-1-replica-2",
						cpuPercent: 41,
						memoryMb: 110,
						deploymentId: "dep-1",
						replica: true,
		...
		const count = await (scalingEngine as any).getCurrentReplicas(
			"proj-1",
			{ server: { id: "server-1" }, mode: "agent" },
			TEST_DEPLOYMENT,
		);
		expect(count).toBe(2);
	});
