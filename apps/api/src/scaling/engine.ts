...
import { getProjectById, getScalingPolicy, listDeployments, listEnvironmentVariablesForDeploy } from "../db/repo";
...
class ScalingEngine {
	...
	private async evaluate(dep: {
		...
		if (!project?.cpuLimit || project.cpuLimit <= 0) return;
		...
		if (stats.cpuPercent < 30) {
			...
			} else if (now - state.lowCpuSince > 300_000) {
				// 5 min
	...
	private async getContainerStats(containerName: string, target: Target): Promise<ContainerStats | null> {
		try {
			...
			if (target.mode === "ssh") {
				const result = await execDockerSshCommand(target.server!, [
					"stats",
					"--no-stream",
					"--format",
					"{{json .}}",
				...
				return {
					...
					memoryMb: this.parseMemToMb(stats.MemUsage?.split("/")[0]?.trim() ?? "0B"),
				};
			}
	...
	private cooldownKey(deploymentId: string) {
		return `dequel:scaling:cooldown:${deploymentId}`;
	}
	...
	private async getCurrentReplicas(
		slug: string,
		target: Target = { server: null, mode: "local" },
		dep: { id: string; projectId: string | null; containerName: string | null } | null = null,
	): Promise<number> {
	...
	private async scaleUp(
		...
		const imageTag =
			target.mode === "ssh"
				? (dep.imageTag ??
					(
						await execDockerSshCommand(target.server!, ["inspect", "-f", "{{.Config.Image}}", dep.containerName!])
					).stdout.trim())
		...
		const envJson =
			target.mode === "ssh"
				? (await execDockerSshCommand(target.server!, ["inspect", "-f", "{{json .Config.Env}}", dep.containerName!]))
						.stdout
	...
	private async updateCaddyRoute(
		...
		const { getIngressServer, shouldRouteViaIngress, projectServerSite, syncIngressRoute, upsertIngressRoute } =
