import { type IngressRouteInfo, removeIngressRouteFile, syncIngressRoute, upsertIngressRoute } from "./ingress";
...
export const syncComposeIngressRoutes = async (
	...
	for (const entry of hostnames) {
		...
		await upsertIngressRoute(ingressServer.id, project.id, deployment.id, workerServer.host, routeInfo);
	}
};
