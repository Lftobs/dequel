...
import { addToCaddyRoute, buildCaddySnippet, removeFromCaddyRoute } from "../domain-verifier";
...
describe("removeFromCaddyRoute", () => {
	...
	it("removes domain from middle of domain list", async () => {
		createRouteFile(
			"elysia",
			"elysia.localhost:80, first.com:80, second.com:80 {\n  reverse_proxy deploy-abc:3000\n}\n",
		);
...
describe("buildCaddySnippet", () => {
	...
	it("creates snippet with verified domains when projectId is provided", async () => {
		...
		expect(snippet).toBe(
			"elysia.localhost:80, app.example.com {\n  log {\n    output stdout\n    format json\n  }\n  reverse_proxy deploy-abc:3000 {\n    header_up Host {upstream_hostport}\n  }\n}\n",
		);
	});
