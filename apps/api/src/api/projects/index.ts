...
export const projectsRoutes = new Elysia()
	...
	.post("/projects/:id/failover", async ({ params: { id }, set }) => {
		try {
	...
	.get("/projects/:id", async ({ params: { id }, set }) => {
	...
	.post("/projects", async ({ body, set }: any) => {
		if (!body?.name) {
			set.status = 400;
		...
		const project = await createProject({
			...
			composeServices: body.composeServices
				? typeof body.composeServices === "string"
					? body.composeServices
					: JSON.stringify(body.composeServices)
				: null,
	...
	.delete("/projects/:id", async ({ params: { id }, set }) => {
	...
	.get("/projects/:id/request-logs", async ({ params: { id }, query: queryParams, set }) => {
		const project = await getProjectById(id);
		if (!project) {
			set.status = 404;
			return fail("Project not found");
	...
	.get("/projects/:id/metrics/requests", async ({ params: { id }, set }) => {
		const project = await getProjectById(id);
		if (!project) {
			set.status = 404;
			return fail("Project not found");
	...
	.get("/projects/:id/request-logs/stream", async ({ params: { id }, request, set }) => {
		const project = await getProjectById(id);
		if (!project) {
			set.status = 404;
			return fail("Project not found");
			...
			async start(controller) {
				...
				try {
					...
					ws.onmessage = (event) => {
						if (closed) return;
						try {
							const dataStr = typeof event.data === "string" ? event.data : new TextDecoder().decode(event.data as any);
