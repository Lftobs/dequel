...
export const prometheusRoutes = new Elysia()
	.get("/prometheus/query", async ({ query, set }: any) => {
		...
		try {
			const url = `http://prometheus:9090/api/v1/query?query=${encodeURIComponent(promQuery)}`;
	...
	.get("/prometheus/query_range", async ({ query, set }: any) => {
		const promQuery = query?.query as string | undefined;
