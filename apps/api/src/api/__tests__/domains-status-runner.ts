...
try {
	...
	await cleanup();
	await pool.query(
		`INSERT INTO projects (id, name, source_type, created_at, updated_at) VALUES ('proj-ds-1', 'Test', 'git', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
	);
	await pool.query(
		`INSERT INTO projects (id, name, source_type, created_at, updated_at) VALUES ('proj-ds-missing', 'NoDomains', 'git', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
	);

	await pool.query(
		`INSERT INTO domains (id, project_id, domain, type, validation_status, ssl_status, created_at) VALUES ('d-ds-1', 'proj-ds-1', 'example.com', 'custom', 'pending', 'pending', NOW()) ON CONFLICT (id) DO NOTHING`,
	);
	await pool.query(
		`INSERT INTO domains (id, project_id, domain, type, validation_status, ssl_status, created_at) VALUES ('d-ds-2', 'proj-ds-1', 'nonexistent.invalid', 'custom', 'pending', 'pending', NOW()) ON CONFLICT (id) DO NOTHING`,
	);
