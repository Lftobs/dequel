...
const reclaimExpiredLeases = async () => {
	try {
		...
		const expiredLeases = await db
			.select()
			.from(agentJobs)
			.where(and(eq(agentJobs.status, "leased"), lt(agentJobs.leaseExpiresAt, now)))
			.execute();
...
const cleanStaleAgents = async () => {
	try {
		...
		const staleAgents = await db
			.select()
			.from(servers)
			.where(and(eq(servers.mode, "agent"), eq(servers.status, "connected"), lt(servers.lastHeartbeat, threshold)))
			.execute();
		...
		for (const server of staleAgents) {
			const updated = await db
				...
				.where(and(eq(servers.id, server.id), eq(servers.status, "connected")))
				.execute();
			...
			const heartbeatAge = server.lastHeartbeat
				...
				: "unknown";
			console.log(
				`[Reconciliation] Marked agent ${server.name} (${server.id}) as disconnected — no heartbeat for ${heartbeatAge}s`,
			);
		}
	} catch (err) {
...
const cleanAbandonedJobs = async () => {
	try {
		...
		const stuckJobs = await db
			.select()
			.from(agentJobs)
			.where(and(eq(agentJobs.status, "running"), lt(agentJobs.startedAt, threshold)))
			.execute();
