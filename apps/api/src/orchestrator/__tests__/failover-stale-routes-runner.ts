...
mock.module(fileUrl('../../db/repo'), () => ({
  ...
  updateRouteStatus: mock((hostname: string, status: string, _lastError: any, serverId?: string) => { routeStatusUpdates.push({ hostname, status, serverId }); return Promise.resolve(); }),
...
mock.module(fileUrl('../../utils/ingress.ts'), () => ({
  ...
  removeIngressRouteFile: mock((_ingressServer: any, info: { hostname: string; routeFile: string }) => { removedRouteFiles.push(info); return Promise.resolve(); }),
...
routesByServer = {
  'srv-a': [
    { id: 'r1', projectId: 'p1', hostname: 'p1.app.com', routeFile: 'p1.conf', status: 'active' },
  ],
};
removedRouteFiles = [];
routeStatusUpdates = [];
...
results.test1_firstTick = { removedEmpty: removedRouteFiles.length === 0, updatesEmpty: routeStatusUpdates.length === 0 };
...
routesByServer = { 'srv-a': [{ id: 'r1', projectId: 'p1', hostname: 'p1.app.com', routeFile: 'p1.conf', status: 'active' }] };
