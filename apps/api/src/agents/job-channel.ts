...
import {
	...
	type AgentContainerStat,
	isRemoteDeployResult,
	type P2PAgentRequest,
} from "./protocol";
...
export const processAgentJobUpdate = async (
	serverId: string,
	update: Exclude<P2PAgentRequest, { type: "p2p_heartbeat" }>,
): Promise<void> => {
				...
				if (payload.projectId) {
					...
					if (current) {
						await updateDeploymentStatus(current.id, "inactive", {
							failureReason: `Superseded by rollback to ${deploymentId.slice(0, 8)}`,
						});
