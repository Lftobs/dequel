...
const PRIVATE_IP_RE =
	/(?::\/\/|^)(?:10\.|172\.(?:1[6-9]|2\d|3[01])\.|192\.168\.|localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\])/;
...
export const validateRemoteDeployment = (deployment: Deployment, project: Project): string | null => {
	...
	if (project.buildType !== "railpack" || project.projectType !== "web")
		return "Remote agents currently support Railpack web services only";
	if (
		project.sourceDir ||
