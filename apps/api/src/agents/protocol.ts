...
export type P2PAgentRequest =
  | { type: "p2p_heartbeat"; protocolVersion: 1; credential: string; agentVersion: string; capabilities: AgentCapabilities; resources?: { cpuUsedPercent?: number; memoryUsedMb?: number }; containers?: AgentContainerStat[] }
  | { type: "job_ack"; protocolVersion: 1; credential: string; jobId: string; leaseId: string }
  | { type: "job_progress"; protocolVersion: 1; credential: string; jobId: string; leaseId: string; stage: string; message: string }
  | { type: "job_result"; protocolVersion: 1; credential: string; jobId: string; leaseId: string; success: boolean; result?: unknown; error?: string };
...
export const isRemoteDeployResult = (value: unknown): value is RemoteDeployResult => {
  ...
  return typeof value.imageTag === "string"
...
const isCapabilities = (value: unknown): value is AgentCapabilities => {
  if (!isRecord(value)) return false;
  return ["docker", "buildkit", "caddy", "compose"].every((key) => typeof value[key] === "boolean")
...
export const parseP2PAgentRequest = (raw: unknown): P2PAgentRequest | null => {
  ...
  if (!isRecord(value) || value.protocolVersion !== AGENT_PROTOCOL_VERSION || typeof value.type !== "string") return null;
  ...
  if (value.type === "job_progress") {
    return typeof value.jobId === "string" && typeof value.leaseId === "string" && typeof value.stage === "string" && typeof value.message === "string"
      ? value as P2PAgentRequest
      : null;
  }
  if (value.type === "job_result") {
    return typeof value.jobId === "string" && typeof value.leaseId === "string" && typeof value.success === "boolean" && (value.error === undefined || typeof value.error === "string")
      ? value as P2PAgentRequest
      : null;
  }
  return null;
