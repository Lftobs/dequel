...
describe("blacklistRefreshToken", () => {
	it("blacklists a refresh token", async () => {
		const { generateRefreshToken, storeRefreshToken, validateRefreshToken, blacklistRefreshToken } = await import(
			"../../utils/auth"
		);
		const token = generateRefreshToken();
		await storeRefreshToken("testuser", token);
...
describe("cleanupExpiredTokens", () => {
	it("removes expired tokens", async () => {
		...
		const [row] = await pool
			.query(`SELECT token_hash FROM refresh_tokens ORDER BY created_at DESC LIMIT 1`)
			.then((r) => r.rows);
		await pool.query(`UPDATE refresh_tokens SET expires_at = '2000-01-01T00:00:00.000Z' WHERE token_hash = $1`, [
			row.token_hash,
		]);
