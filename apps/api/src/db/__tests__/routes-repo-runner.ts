...
const cleanupRoutes = async () => {
	await pool.query(
		`DELETE FROM routes WHERE hostname IN ('app.localhost:80', 'other.localhost:80', 'shared.localhost:80')`,
	);
	await pool.query(`DELETE FROM projects WHERE id IN ('proj-r1', 'proj-r2', 'proj-r3')`);
};

try {
	const { upsertRoute, listRoutes, getRouteByHostname, updateRouteStatus, deleteRoutesByDeployment } = await import(
		"../repo/routes"
	);

	await cleanupRoutes();
	await pool.query(
		`INSERT INTO projects (id, name, source_type, created_at, updated_at) VALUES ('proj-r1', 'Test', 'git', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
	);
	await pool.query(
		`INSERT INTO projects (id, name, source_type, created_at, updated_at) VALUES ('proj-r2', 'Test2', 'git', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
	);
	await pool.query(
		`INSERT INTO projects (id, name, source_type, created_at, updated_at) VALUES ('proj-r3', 'Test3', 'git', NOW(), NOW()) ON CONFLICT (id) DO NOTHING`,
	);
