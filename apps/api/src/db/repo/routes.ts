	...
	const existing = input.serverId
		? (
				await db
					.select()
					.from(routes)
					.where(and(eq(routes.hostname, input.hostname), eq(routes.serverId, input.serverId)))
					.execute()
			)[0]
		: (
				await db
					.select()
					.from(routes)
					.where(and(eq(routes.hostname, input.hostname), isNull(routes.serverId)))
					.execute()
			)[0];
	...
	const row = serverId
		? (
				await db
					.select()
					.from(routes)
					.where(and(eq(routes.hostname, hostname), eq(routes.serverId, serverId)))
					.execute()
			)[0]
		: (
				await db.select().from(routes).where(eq(routes.hostname, hostname)).orderBy(desc(routes.createdAt)).execute()
			)[0];
...
export const listIngressRoutes = async (): Promise<Route[]> => {
	...
	const rows = await db
		.select()
		.from(routes)
		.where(and(isNotNull(routes.upstreamHost), eq(routes.status, "active")))
		.execute();
	return rows.map(mapRoute);
};
