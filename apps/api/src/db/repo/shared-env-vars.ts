...
import type { CreateSharedEnvVarInput, SharedEnvVar } from "../../types";
import { config } from "../../utils/config";
import { decryptValue, encryptValue } from "../../utils/crypto";
import { getDb } from "../db-provider";
import { projectSharedEnvLinks, sharedEnvVars } from "../schema";
...
export const listLinkedSharedEnvVars = async (projectId: string): Promise<SharedEnvVar[]> => {
	...
	const links = await db
		...
		.orderBy(asc(projectSharedEnvLinks.createdAt))
		.execute();
	...
	const rows = await db
		.select()
		.from(sharedEnvVars)
		.where(ids.length === 1 ? eq(sharedEnvVars.id, ids[0]) : undefined)
		.execute();
...
export const listSharedEnvVarsForDeploy = async (
	...
	const filtered = environment
		? linked.filter((v) => v.environment === "production" || v.environment === environment)
		: linked;
