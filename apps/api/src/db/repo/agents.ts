...
const allocatePeerIp = async (): Promise<string> => {
	const db = await getDb();
	const rows = await db
		.select({ peerIp: servers.peerIp })
		.from(servers)
		.where(and(eq(servers.mode, "agent"), isNull(servers.revokedAt)))
		.execute();
...
export const exchangeAgentRegistrationToken = async (
	...
	const consumed = await db
		...
		.where(and(eq(agentRegistrationTokens.id, registration.id), isNull(agentRegistrationTokens.usedAt)))
		.execute();
...
export const validateAgentCredential = async (rawCredential: string) => {
	...
	const [row] = await db
		...
		.where(and(eq(agentCredentials.credentialHash, hashToken(rawCredential)), isNull(agentCredentials.revokedAt)))
		.execute();
	if (!row) return null;
...
export const updateAgentHeartbeat = async (
	...
	await db
		.update(servers)
		.set(updates)
		.where(and(eq(servers.id, serverId), eq(servers.mode, "agent"), isNull(servers.revokedAt)))
		.execute();
};
