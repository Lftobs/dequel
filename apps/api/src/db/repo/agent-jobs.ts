...
export const leaseNextAgentJob = async (serverId: string, leaseMs = 30_000): Promise<AgentJobEnvelope | null> => {
  ...
  )).orderBy(agentJobs.createdAt).execute();
  if (!row) return null;
  const leaseExpiresAt = new Date(Date.now() + leaseMs);
  const leaseId = randomUUID();
  const sameLease = row.leaseId ? eq(agentJobs.leaseId, row.leaseId) : isNull(agentJobs.leaseId);
  const leased = await db.update(agentJobs).set({
    status: "leased",
    attempts: row.attempts + 1,
    leaseId,
    leaseExpiresAt,
  }).where(and(eq(agentJobs.id, row.id), eq(agentJobs.status, row.status), sameLease)).execute();
...
export const acknowledgeAgentJob = async (jobId: string, serverId: string, leaseId: string) => {
  const db = await getDb();
  const result = await db.update(agentJobs).set({ status: "running", startedAt: now() })
    .where(and(eq(agentJobs.id, jobId), eq(agentJobs.serverId, serverId), eq(agentJobs.leaseId, leaseId), eq(agentJobs.status, "leased"))).execute();
  return getRowsAffected(result) === 1;
};

export const getAgentJobDeploymentId = async (jobId: string, serverId: string, leaseId: string) => {
  const db = await getDb();
  const [row] = await db.select({ deploymentId: agentJobs.deploymentId }).from(agentJobs)
    .where(and(eq(agentJobs.id, jobId), eq(agentJobs.serverId, serverId), eq(agentJobs.leaseId, leaseId), eq(agentJobs.status, "running"))).execute();
  return row?.deploymentId ?? null;
};

export const getAgentJobInfo = async (jobId: string, serverId: string, leaseId: string) => {
  const db = await getDb();
  const [row] = await db.select({ deploymentId: agentJobs.deploymentId, type: agentJobs.type, payload: agentJobs.payload }).from(agentJobs)
    .where(and(eq(agentJobs.id, jobId), eq(agentJobs.serverId, serverId), eq(agentJobs.leaseId, leaseId), eq(agentJobs.status, "running"))).execute();
  if (!row) return null;
...
export const finishAgentJob = async (jobId: string, serverId: string, leaseId: string, success: boolean, error?: string) => {
  const db = await getDb();
  const result = await db.update(agentJobs).set({
    status: success ? "succeeded" : "failed",
    failureReason: success ? null : error || "Agent job failed",
    finishedAt: now(),
    leaseId: null,
    leaseExpiresAt: null,
  }).where(and(eq(agentJobs.id, jobId), eq(agentJobs.serverId, serverId), eq(agentJobs.leaseId, leaseId), or(eq(agentJobs.status, "running"), eq(agentJobs.status, "cancelled")))).execute();
  return getRowsAffected(result) === 1;
};
...
export const requeueRunningAgentJobs = async (serverId: string) => {
  ...
  const result = await db.update(agentJobs).set({
    ...
    leaseExpiresAt: null,
    startedAt: null,
  }).where(and(eq(agentJobs.serverId, serverId), eq(agentJobs.status, "running"))).execute();
  return getRowsAffected(result);
};
