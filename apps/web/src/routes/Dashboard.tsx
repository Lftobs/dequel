...
import { Activity, Clock, FolderKanban, GitBranch, Globe, Layers, Plus, Search, Server, Trash2 } from "lucide-react";
	...
	return (
		<div className="space-y-8 max-w-[1400px] mx-auto">
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-[#1c1c21]">
				<div>
					<h1 className="text-2xl font-bold text-zinc-100 tracking-tight flex items-center gap-2">Overview</h1>
					...
					<div className="rounded-xl border border-[#1c1c21] bg-[#0c0c0e] p-5 space-y-4">
						...
						{deploymentsLoading ? (
							...
							<div className="space-y-3.5">
								{allDeployments
									.filter(
										(dep) => dep.status === "running" && dep.projectId && projects.some((p) => p.id === dep.projectId),
									)
									.slice(0, 5)
									.map((dep) => {
					...
					{filteredProjects.length === 0 ? (
						...
						<div className="grid gap-4 md:grid-cols-2">
							{filteredProjects.map((project) => (
								<ProjectGridCard key={project.id} project={project} onDelete={() => deleteProject.mutate(project.id)} />
							))}
						</div>
					)}
				</div>
				...
				<DialogContent className="sm:max-w-[400px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
					<DialogHeader>
						...
						<DialogDescription className="text-xs text-muted-foreground mt-2">
							...
							<span className="font-semibold text-foreground font-mono">{project.name}</span>? This action cannot be
