			...
			<CardContent className="space-y-5 p-5">
				<div className="flex items-start justify-between gap-4">
					<div>
						<div className="mb-2 flex items-center gap-2">
							<Badge
								variant="outline"
								className="border-amber-500/20 bg-amber-500/5 text-[10px] uppercase text-amber-400"
							>
								{database.type}
							</Badge>
						...
						<p className="mt-1 text-xs text-zinc-500">
							{project?.name ?? "Standalone"} · {database.databaseName}
						</p>
					</div>
					<Button
						variant="ghost"
				...
				<div>
					<div className="mb-2 flex justify-between text-xs">
						...
						<span
							className={
								storagePercent >= 100 ? "text-red-400" : storagePercent >= 80 ? "text-amber-400" : "text-zinc-300"
							}
						>
								...
								<button
									...
									onClick={() =>
										navigator.clipboard.writeText(
											credentials.externalConnectionString ?? credentials.internalConnectionString,
										)
									}
									className="p-1 text-zinc-500 hover:text-zinc-200"
				...
				<div className="flex gap-2 border-t border-border pt-4">
					{database.status === "stopped" ? (
						<Button size="sm" disabled={isBusy} onClick={() => lifecycle("start")}>
							<Play className="mr-2 h-3.5 w-3.5" /> Start
						</Button>
					) : database.status === "failed" ? (
						<Button size="sm" disabled={isBusy} onClick={() => lifecycle("retry")}>
