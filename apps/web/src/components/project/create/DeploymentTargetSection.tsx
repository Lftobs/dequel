						...
						{targets.map((s) => (
							<SelectItem key={s.id} value={s.id}>
								<span className="flex items-center gap-2">
									...
									<span className="font-medium">{s.mode === "local" ? "This Machine" : s.name}</span>
		...
		<div className={cn("space-y-4 bg-[#0d0d10] border border-[#1f1f26] p-5 sm:p-6 rounded-2xl shadow-lg", className)}>
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-zinc-400 mt-1">Choose which server runs this project's deployments.</p>
				</div>
			</div>
