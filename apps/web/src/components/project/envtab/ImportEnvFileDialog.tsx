...
export function ImportEnvFileDialog({ open, onOpenChange, onImport }: ImportEnvFileDialogProps) {
	const [parsedFileVars, setParsedFileVars] = useState<{ key: string; value: string }[] | null>(null);
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[450px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
				<DialogHeader>
					<DialogTitle className="text-lg font-bold text-foreground">Import .env File</DialogTitle>
				...
				<div className="space-y-4 pt-2">
					<div className="relative border-2 border-dashed border-border rounded-xl p-6 hover:border-primary/40 transition-colors bg-[#0d0d11]/40 flex flex-col items-center justify-center text-center group">
						...
						<p className="text-xs font-semibold text-foreground mb-1">Select a file or drag here</p>
						<p className="text-[10px] text-muted-foreground">Supports .env or .txt key-value files</p>
						<Input
							key={fileInputKey}
							type="file"
							accept=".env,.txt"
					...
					{parsedFileVars && importStatus !== "done" && (
						<div className="space-y-3">
							<div className="flex items-center justify-between text-xs">
								<span className="text-muted-foreground">Parsed variables:</span>
								<span className="font-semibold text-primary">{parsedFileVars.length} found</span>
							</div>
							<div className="max-h-[140px] overflow-y-auto border border-border rounded-lg bg-[#070709] divide-y divide-border/40 font-mono text-[10px]">
								{parsedFileVars.slice(0, 10).map((v, idx) => (
									<div key={idx} className="flex items-center justify-between p-2">
										<span className="text-foreground font-semibold truncate max-w-[150px]">{v.key}</span>
										<span className="text-muted-foreground truncate max-w-[200px]">{v.value}</span>
									</div>
								))}
					...
					{importStatus === "done" && (
						<div className="text-xs text-emerald-400 flex items-center justify-center gap-1.5 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 font-semibold">
							<Check className="h-4 w-4" /> Successfully Imported Variables!
						</div>
					)}
					...
					<div className="flex justify-end gap-2 pt-2 border-t border-border/40">
						...
						{parsedFileVars && importStatus !== "done" && (
							...
							>
								{importStatus === "importing" ? "Importing..." : `Import ${parsedFileVars.length} Variables`}
							</Button>
						)}
					</div>
				</div>
