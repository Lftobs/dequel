...
export function VolumesTab({ projectId }: VolumesTabProps) {
	...
	const latestDeployment = deploymentsData?.items?.[0];
	...
	const add = async (e: React.FormEvent) => {
		...
		setIsAdding(true);
		try {
			await api.createVolume(projectId, mountPath.trim());
	...
	const selectedVolToDelete = volumes.find((v) => v.id === deletingVolId);
		...
		<div className="space-y-6">
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-amber-500/20 bg-amber-500/5 gap-3 animate-in fade-in-0 duration-200">
					<div className="flex items-start gap-3">
						<div className="w-8 h-8 rounded-lg bg-amber-500/10 text-amber-500 flex items-center justify-center shrink-0 border border-amber-500/15">
							<RefreshCw className={`h-4 w-4 ${redeploy.isPending ? "animate-spin" : ""}`} />
						</div>
						<div>
							<h4 className="text-sm font-semibold text-foreground">Redeployment Required</h4>
							<p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
								You have added or deleted persistent volumes. Redeploy your application instances to mount these storage
								pathways.
							</p>
						</div>
					</div>
					<div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
						...
						<Button
							...
							onClick={async () => {
								try {
									await redeploy.mutateAsync(latestDeployment.id);
			...
			{volumes.length === 0 ? (
				<div className="flex flex-col items-center justify-center border border-dashed border-border rounded-2xl p-12 text-center bg-card/20 backdrop-blur-sm relative overflow-hidden group min-h-[350px]">
					...
					<h3 className="text-lg font-semibold text-foreground mb-2">No Persistent Volumes</h3>
					<p className="text-sm text-muted-foreground max-w-sm mb-6 leading-relaxed">
						Attach persistent disk pathways to your project. Mounted pathways preserve data across scaling actions,
			...
			<Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
				<DialogContent className="sm:max-w-[420px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
					<DialogHeader>
						<DialogTitle className="text-lg font-bold text-foreground">Add Persistent Volume</DialogTitle>
						<DialogDescription className="text-xs text-muted-foreground">
							Define a persistent path inside your container.
						</DialogDescription>
					</DialogHeader>
			...
			<Dialog open={deletingVolId !== null} onOpenChange={(open) => !open && setDeletingVolId(null)}>
				<DialogContent className="sm:max-w-[400px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
					<DialogHeader>
						<DialogTitle className="text-lg font-bold text-foreground">Unmount Volume</DialogTitle>
						<DialogDescription className="text-xs text-muted-foreground mt-2">
							Are you sure you want to unmount volume{" "}
							<span className="font-semibold text-foreground font-mono">{selectedVolToDelete?.mountPath}</span>? This
							will break connection to storage inside container deployments. Raw data remains on cluster node volumes
							but is unlinked.
						</DialogDescription>
					</DialogHeader>
