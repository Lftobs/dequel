...
export function DomainsTab({ projectId }: DomainsTabProps) {
	...
	const [deletingDomId, setDeletingDomId] = useState<string | null>(null);
	...
	const dnsName = lastAdded ? (lastAdded.split(".").length > 2 ? lastAdded.split(".")[0] : "@") : "";

	const selectedDomToDelete = domains.find((d) => d.id === deletingDomId);
	...
	return (
		<div className="space-y-6">
			{domains.length === 0 ? (
				<div className="flex flex-col items-center justify-center border border-dashed border-border rounded-2xl p-12 text-center bg-card/20 backdrop-blur-sm relative overflow-hidden group min-h-[350px]">
					...
					<p className="text-sm text-muted-foreground max-w-sm mb-6 leading-relaxed">
						Point web traffic to your container deployments. Attach custom domain addresses with automatic Let's Encrypt
						SSL provisioning.
					</p>
					<Button
				...
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-sm text-muted-foreground">Domains and subdomains linked to project endpoints.</p>
						</div>
						<Button
							onClick={() => setIsAddOpen(true)}
						...
						>
							<Plus className="h-4.5 w-4.5" /> Add Domain
						</Button>
					</div>
								...
								<div className="flex items-start gap-3.5">
									...
									<div className="space-y-2.5 text-sm w-full">
										<p className="font-semibold text-foreground">
											DNS Configuration Required for <code className="text-primary font-mono">{lastAdded}</code>
										</p>
										{hasBaseDomain ? (
											<>
													...
													<table className="w-full">
														<thead>
															<tr className="bg-[#0b0b0f] text-left">
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Type</th>
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Host/Name</th>
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Target/Value</th>
															</tr>
														</thead>
														<tbody>
															<tr className="border-t border-border/40">
																<td className="px-3 py-2 text-foreground font-bold">CNAME</td>
																<td className="px-3 py-2 text-foreground">{dnsName}</td>
																<td className="px-3 py-2 text-foreground flex items-center justify-between">
																	<span className="truncate max-w-[200px]">{baseDomain}</span>
																	<button
																		type="button"
																		onClick={() => copyText(baseDomain)}
											...
											<>
												<p className="text-xs text-muted-foreground">Add an A record at your DNS domain registrar:</p>
												<div className="rounded-lg border border-border bg-[#070709] overflow-hidden text-[10px] font-mono w-full">
													<table className="w-full">
														<thead>
															<tr className="bg-[#0b0b0f] text-left">
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Type</th>
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Host/Name</th>
																<th className="px-3 py-1.5 text-muted-foreground font-semibold">Value</th>
															</tr>
														</thead>
														<tbody>
															<tr className="border-t border-border/40">
																<td className="px-3 py-2 text-foreground font-bold">A</td>
																<td className="px-3 py-2 text-foreground">{dnsName}</td>
																<td className="px-3 py-2 text-foreground flex items-center justify-between">
																	<span>{serverIp?.ip ?? "../.."}</span>
																	{serverIp?.ip && (
																		<button
																			type="button"
																			onClick={() => copyText(serverIp.ip)}
																			className={`p-1 transition-colors ${copiedTarget ? "text-emerald-400" : "text-muted-foreground hover:text-foreground"}`}
																		>
																			{copiedTarget ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
																		</button>
										...
										<p className="text-[10px] text-muted-foreground/80 leading-relaxed">
											DNS changes can take up to 48 hours to propagate globally. Dequel will validate ssl cert status
											automatically.
										</p>
									</div>
								</div>
						...
						<Table className="min-w-[600px] md:min-w-full">
							<TableHeader className="bg-[#0b0b0f]/50">
								<TableRow className="border-border hover:bg-transparent">
									<TableHead className="text-xs font-semibold py-3">Domain</TableHead>
									<TableHead className="text-xs font-semibold py-3">Type</TableHead>
									<TableHead className="text-xs font-semibold py-3">Validation</TableHead>
									<TableHead className="text-xs font-semibold py-3">SSL</TableHead>
									<TableHead className="text-xs font-semibold py-3">DNS</TableHead>
									<TableHead className="text-xs font-semibold py-3">TLS</TableHead>
									<TableHead className="w-12 text-right pr-6"></TableHead>
								</TableRow>
							</TableHeader>
							<TableBody>
								{domains.map((d) => (
									<TableRow key={d.id} className="border-border hover:bg-[#121217]/30 group transition-all">
										<TableCell className="font-semibold py-3.5 text-foreground">{d.domain}</TableCell>
			...
			<Dialog open={deletingDomId !== null} onOpenChange={(open) => !open && setDeletingDomId(null)}>
				<DialogContent className="sm:max-w-[400px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
					<DialogHeader>
						<DialogTitle className="text-lg font-bold text-foreground">Remove Domain</DialogTitle>
						<DialogDescription className="text-xs text-muted-foreground mt-2">
							Are you sure you want to remove domain{" "}
							<span className="font-semibold text-foreground font-mono">{selectedDomToDelete?.domain}</span>? This will
							stop routing incoming traffic from this path to your running container service.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter className="flex justify-end gap-2 pt-4 border-t border-border/40">
						<Button
							variant="ghost"
							onClick={() => setDeletingDomId(null)}
