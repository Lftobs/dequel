...
import { useRuntimeLogs, useRequestLogs } from "../../hooks/useDeploymentLogs";
...
export function LogsTab({ projectId }: LogsTabProps) {
	...
	const [logSource, setLogSource] = useState<"runtime" | "request">("request");
	...
	const { logs: runtimeLogs, isLoading: isRuntimeLoading, refetch: refetchRuntime } = useRuntimeLogs(latest?.id || null, isLive && logSource === "runtime");
	const { logs: requestLogs, isLoading: isRequestLoading, refetch: refetchRequest } = useRequestLogs(projectId, isLive && logSource === "request", startMs, endMs);
		...
		if (message.startsWith("{") && message.endsWith("}")) {
			try {
				...
				if (obj.request) {
					...
					const durMs = obj.duration ? `${(obj.duration * 1000).toFixed(2)}ms` : "0ms";
					const bytes = obj.size ? `${obj.size} B` : "0 B";
					message = `Status: ${obj.status || "—"} • Duration: ${durMs} • Size: ${bytes}`;
				} else {
	...
	// Group logs into 1-minute bins for the last 30 minutes to make a histogram
	const makeHistogram = () => {
		...
		<div className="space-y-6">
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border border-[#1a1a1f] bg-[#0c0c0e] rounded-xl select-none">
				...
				<div className="flex items-center gap-3">
					...
					>
						<span className={cn("w-1.5 h-1.5 rounded-full bg-amber-500", isLive && !(startDate !== "" || endDate !== "") && "animate-pulse")} />
				...
				<div className="h-12 flex items-end gap-1.5 pt-4 select-none">
					{bins.map((bin, idx) => (
						<div
							...
							style={{ height: `${(bin.count / maxCount) * 100}%`, minHeight: "2px" }}
						>
									...
									filteredLogs.map((log, idx) => (
										...
										>
											<td className="py-2 px-4 text-zinc-500 whitespace-nowrap">
												{new Date(
													(log as any).timestamp || log.createdAt,
												).toLocaleTimeString()}
											</td>
						...
						<div className="space-y-3 font-mono text-[11px]">
							<div className="space-y-1">
								...
								<div className="text-zinc-300">
									{new Date(
										(selectedLog as any).timestamp || selectedLog.createdAt,
									).toLocaleString()}
								</div>
							</div>
