...
import { useRequestLogs, useRuntimeLogs } from "../../../hooks/useDeploymentLogs";
...
export function LogsTab({ projectId }: LogsTabProps) {
	...
	const [logSource, setLogSource] = useState<"runtime" | "request">("runtime");
	...
	} = useRuntimeLogs(latest?.id || null, isLive && logSource === "runtime");
	const {
		logs: requestLogs,
		isLoading: isRequestLoading,
		refetch: refetchRequest,
	} = useRequestLogs(projectId, isLive && logSource === "request", startMs, endMs);

	const logs = logSource === "runtime" ? runtimeLogs : requestLogs;
	const isLoading = logSource === "runtime" ? isRuntimeLoading : isRequestLoading;
	const refetch = logSource === "runtime" ? refetchRuntime : refetchRequest;
	...
	const parsedLogs = logs.map((log) => {
		...
		if (message.startsWith("{") && message.endsWith("}")) {
			...
			if (upper.includes("ERROR") || upper.includes("CRITICAL") || upper.includes("FAIL")) level = "error";
	...
	const makeHistogram = () => {
		...
		const bins = Array.from({ length: 30 }, (_, idx) => {
			const binStart = nowMs - (30 - idx) * 60000;
		...
		for (const log of filteredLogs) {
			const time = new Date((log as any).timestamp || log.createdAt).getTime();
					...
					>
						<span
							className={cn(
								...
								isLive && !(startDate !== "" || endDate !== "") && "animate-pulse",
							)}
						/>
			...
			{logSource === "request" && (
				<div className="flex flex-wrap items-center gap-4 bg-[#0a0a0c] border border-[#1a1a1f] p-3 rounded-xl text-xs select-none">
					<div className="flex items-center gap-2">
						<span className="text-zinc-500 font-semibold">Date Filter:</span>
					</div>
					<div className="flex flex-wrap items-center gap-4">
						<div className="flex items-center gap-2">
			...
			<div className="flex gap-4 items-start relative min-h-[400px]">
									...
									filteredLogs.map((log, idx) => (
										...
										>
											<td className="py-2 px-4 text-zinc-500 whitespace-nowrap">
												{new Date((log as any).timestamp || log.createdAt).toLocaleTimeString()}
											</td>
											...
											{logSource === "request" && (
												<>
													...
													<td className="py-2 px-3 text-zinc-400 truncate max-w-[110px]">{log.host}</td>
													<td className="py-2 px-3 text-zinc-400 truncate max-w-[140px]">{log.request}</td>
												</>
											)}
											<td className="py-2 px-4 text-zinc-300 break-all max-w-lg truncate">{log.parsedMessage}</td>
										</tr>
									))
								)}
							</tbody>
				...
				{selectedLog && (
					<div className="w-[360px] border border-[#27272a] bg-[#111113] rounded-xl p-5 space-y-4 animate-in slide-in-from-right-3 duration-250 shrink-0 shadow-2xl relative">
						...
						<div>
							<h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Log Event Details</h4>
							<span
								className={cn(
						...
						<div className="space-y-3 font-mono text-[11px]">
							<div className="space-y-1">
								<div className="text-zinc-500 text-[10px] uppercase font-sans">Timestamp</div>
							...
							{logSource === "request" && selectedLog.status && (
								<div className="space-y-1">
									<div className="text-zinc-500 text-[10px] uppercase font-sans">Status</div>
							...
							{logSource === "request" && selectedLog.request && (
								<div className="space-y-1">
									<div className="text-zinc-500 text-[10px] uppercase font-sans">Request</div>
									<div className="text-zinc-300">{selectedLog.request}</div>
								</div>
							)}

							{logSource === "request" && selectedLog.duration && (
								<div className="space-y-1">
									<div className="text-zinc-500 text-[10px] uppercase font-sans">Duration</div>
									<div className="text-zinc-300">{selectedLog.duration}</div>
								</div>
							)}

							{logSource === "request" && selectedLog.size && (
								<div className="space-y-1">
									<div className="text-zinc-500 text-[10px] uppercase font-sans">Size</div>
									<div className="text-zinc-300">{selectedLog.size}</div>
								</div>
							)}

							<div className="space-y-1">
								<div className="text-zinc-500 text-[10px] uppercase font-sans">Message</div>
								<div className="text-zinc-300 break-all">{selectedLog.parsedMessage}</div>
							</div>

							<div className="space-y-1">
								<div className="text-zinc-500 text-[10px] uppercase font-sans">Raw JSON payload</div>
