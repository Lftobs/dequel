...
import { Cpu, Sliders, Trash2, TrendingUp } from "lucide-react";
...
export function ScalingTab({ projectId }: ScalingTabProps) {
	...
	const matchedPreset = PRESETS.find((p) => p.cpu === cpuLimit && p.memory === memoryLimitMb);
	...
	return (
		<div className="space-y-6">
			...
			<Card className="bg-card/40 border-border backdrop-blur-sm">
				<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
					<div className="space-y-1">
						...
						<p className="text-xs text-muted-foreground">Set custom limits or select a package for containers.</p>
					</div>
					{project?.cpuLimit || project?.memoryLimitMb ? (
						...
						<Badge variant="outline" className="border-muted text-muted-foreground uppercase text-[10px]">
							Unlimited
						</Badge>
					)}
				</CardHeader>
					...
					<div className="grid gap-4 sm:grid-cols-2">
						<div className="grid gap-2">
							...
							<div className="relative flex items-center">
								...
								/>
								<span className="absolute right-3 text-xs font-semibold text-muted-foreground">cores</span>
							</div>
						</div>
						<div className="grid gap-2">
							<label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
							...
							<div className="relative flex items-center">
								...
								/>
								<span className="absolute right-3 text-xs font-semibold text-muted-foreground">MB</span>
							</div>
						</div>
					</div>
				...
				<CardContent>
					{!policy && !isConfiguring ? (
						<div className="flex flex-col items-center justify-center py-6 text-center">
							...
							<h4 className="text-sm font-semibold text-foreground mb-1">Auto-scaling is not configured</h4>
							<p className="text-xs text-muted-foreground max-w-sm mb-4 leading-relaxed">
								Automatically adjust replica counts between min/max limits based on average CPU usage. Perfect for load
								spikes.
							</p>
							<Button
							...
							<div className="grid gap-4 sm:grid-cols-3">
								...
								<div className="grid gap-2">
									...
									<div className="relative flex items-center">
										...
										/>
										<span className="absolute right-3 text-xs font-semibold text-muted-foreground">%</span>
									</div>
								</div>
							</div>
						...
						<div className="space-y-6">
							...
							<div className="flex items-center justify-between pt-2 border-t border-border/40">
								<Button
									variant="outline"
									onClick={() => setIsDisableScalingOpen(true)}
			...
			<Dialog open={isDisableScalingOpen} onOpenChange={setIsDisableScalingOpen}>
				<DialogContent className="sm:max-w-[400px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
					<DialogHeader>
						<DialogTitle className="text-lg font-bold text-foreground">Disable Auto-scaling</DialogTitle>
						<DialogDescription className="text-xs text-muted-foreground mt-2">
							Are you sure you want to disable auto-scaling for this project? This will stop adjusting container
							replicas automatically based on CPU usage.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter className="flex justify-end gap-2 pt-4 border-t border-border/40">
						<Button
							variant="ghost"
							onClick={() => setIsDisableScalingOpen(false)}
							className="h-9 text-xs px-4 rounded-lg hover:bg-[#1a1a21]"
						>
							Cancel
						</Button>
