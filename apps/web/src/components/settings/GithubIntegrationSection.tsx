        ...
        <form onSubmit={save} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              ...
              <Input placeholder="Iv1..." value={clientId} onChange={(e) => setClientId(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Client Secret</label>
              <Input type="password" placeholder={data?.configured ? '(unchanged)' : ''} value={clientSecret} onChange={(e) => setClientSecret(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">App Name</label>
              <Input placeholder="Dequel" value={appName} onChange={(e) => setAppName(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Webhook Secret</label>
              <Input type="password" placeholder={data?.hasWebhookSecret ? '(unchanged)' : ''} value={webhookSecret} onChange={(e) => setWebhookSecret(e.target.value)} />
            </div>
          </div>
          <div className="pt-1">
