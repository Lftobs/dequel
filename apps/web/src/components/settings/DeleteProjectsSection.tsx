...
export function DeleteProjectsSection() {
	...
	return (
		<Card className="border-red-500/20 bg-card/60 backdrop-blur-sm shadow-sm">
									...
									<TableRow key={p.id} className="border-border/40 hover:bg-white/[0.02]">
										...
										<TableCell>
											<div className="flex items-center gap-1.5 flex-wrap">
												<Badge variant="outline" className="text-[10px] uppercase font-mono py-0 px-1.5 bg-muted/20 border-border">
													{p.projectType || "web"}
												</Badge>
												{p.repoUrl ? (
													<span className="text-[11px] text-muted-foreground font-mono truncate max-w-[160px]" title={p.repoUrl}>
			...
			<Dialog open={deletingProject !== null} onOpenChange={(open) => { if (!open) setDeletingProject(null); }}>
					...
					<div className="space-y-3 py-2">
						<div className="space-y-1.5">
							<label className="text-xs font-medium text-muted-foreground">
								To confirm, please type <strong className="text-foreground select-all">{deletingProject?.name}</strong> below:
							</label>
							<Input
