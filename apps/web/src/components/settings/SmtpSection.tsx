        ...
        <form onSubmit={save} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Input type="password" placeholder={data?.configured ? '(unchanged)' : ''} value={pass} onChange={(e) => setPass(e.target.value)} />
            </div>
            <div className="grid gap-1.5 sm:col-span-2">
              <label className="text-xs font-medium text-muted-foreground">From Address</label>
              <Input placeholder="dequel@example.com" value={fromAddress} onChange={(e) => setFromAddress(e.target.value)} />
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <Button type="submit" size="sm" className="w-full sm:w-auto">Save</Button>
            <Button type="button" size="sm" variant="secondary" onClick={test} disabled={!data?.configured} className="w-full sm:w-auto">
              Test Connection
            </Button>
          </div>
        </form>
