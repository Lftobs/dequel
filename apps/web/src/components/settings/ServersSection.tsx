  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        <form onSubmit={addSshServer} className="rounded-lg border border-border bg-black/20 p-4 space-y-4">
          <div className="flex items-start gap-3">
            <Server className="mt-0.5 h-4 w-4 text-orange-500 shrink-0" />
            <div>
          ...
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-muted-foreground">
              SSH Private Key <span className="text-muted-foreground/60">(optional — paste PEM content)</span>
            </label>
            <textarea
          ...
          <div className="flex justify-end">
            <Button type="submit" size="sm" className="bg-orange-500 hover:bg-orange-600 text-white font-semibold w-full sm:w-auto">
              Add SSH Server
            </Button>
          </div>
        </form>
        ...
        <details className="rounded-lg border border-border px-4 py-3">
          ...
          <form onSubmit={createRegistration} className="mt-3 space-y-3">
            <p className="text-xs text-muted-foreground">
              Establishes an encrypted Direct P2P WireGuard tunnel for firewalled nodes or homelabs without open SSH ports.
            </p>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              ...
              <Button type="submit" size="sm" variant="outline" className="w-full sm:w-auto">Generate P2P Agent Command</Button>
            </div>
            {registrationCommand && (
              ...
              {servers.map((s) => (
                <div key={s.id} className="p-3.5 space-y-2.5">
                  ...
                  <p className="text-xs font-mono text-muted-foreground break-all">
                    {s.mode === 'agent' ? `P2P WireGuard Agent ${s.agentVersion || ''}` : `SSH (${s.sshUser || 'root'}@${s.host}:${s.port})${s.sshKey ? ' [key]' : ''}`}
                  </p>
        ...
        {preparingId && (
          <div className="rounded-lg border border-border bg-black/30 p-3 space-y-2">
            <div className="flex items-center gap-2">
        ...
        {prepareDone && (
          <div className={`rounded-lg border p-3 text-xs ${prepareError ? 'border-red-500/40 text-red-400' : 'border-emerald-500/40 text-emerald-400'}`}>
      ...
      <Dialog open={deletingServerId !== null} onOpenChange={(open) => { if (!open) setDeletingServerId(null); }}>
        <DialogContent className="sm:max-w-[400px] bg-card border-border text-foreground rounded-2xl shadow-2xl">
          ...
          <DialogFooter className="flex justify-end gap-2 pt-4 border-t border-border/40">
            <Button variant="ghost" onClick={() => setDeletingServerId(null)} className="h-10 text-xs px-4 rounded-xl hover:bg-[#1a1a21]">
              Cancel
            </Button>
            <Button onClick={handleDeleteServer} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground font-semibold h-10 text-xs px-5 rounded-xl shadow-lg transition-all">
              Remove Server
            </Button>
          </DialogFooter>
        </DialogContent>
