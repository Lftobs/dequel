...
import { disconnectGithub, getGithubRepos, getRepoHooks, registerRepoHook, removeRepoHook } from "../../api/client";
...
export function RepoPicker({ onSelect, selected, onDisconnect, onConnect }: RepoPickerProps) {
	...
	const toggleWebhook = async () => {
		...
		try {
			...
			setWebhookError(
				message.includes("Not authenticated") ? "GitHub session expired. Reconnect GitHub, then try again." : message,
			);
		} finally {
			...
			<div className="space-y-2 w-full max-w-full">
				<div className="flex items-center gap-2 p-3 rounded-xl bg-[#0a0a0d] border border-[#222227]">
					...
					<div className="flex-1 min-w-0">
						<div className="text-xs font-semibold text-zinc-200 truncate">{selected.fullName}</div>
				...
				{webhookError && <p className="text-[10px] text-red-400">{webhookError}</p>}
			</div>
		);
	}
	...
	return (
		<div className="space-y-3 w-full max-w-full overflow-hidden">
			...
			{loading ? (
				...
				<div className="max-h-64 overflow-y-auto overflow-x-hidden space-y-1 pr-1 w-full">
					{filtered.map((repo) => (
						<button
							type="button"
							key={repo.id}
							onClick={() => onSelect(repo)}
							className="w-full max-w-full text-left p-2.5 rounded-lg hover:bg-[#141418] border border-transparent hover:border-[#222227] transition-all flex items-start gap-2.5 group"
						>
							...
							<div className="flex-1 min-w-0">
								...
								{repo.description && <p className="text-[10px] text-zinc-500 truncate mt-0.5">{repo.description}</p>}
								<div className="flex items-center gap-2 mt-1">
									...
									<span className="text-[9px] text-zinc-600">{repo.defaultBranch}</span>
								</div>
							</div>
						</button>
					))}
